        ...
        for g in range(len(ghosts)):
            ...
            if ghosts[g].collidepoint((player.x, player.y)):
                if ghosts[g].status > 0:
                    player.score += 100
                    moveGhosts()
                    animate(ghosts[g], pos=(290, 370), duration=1/Ghost_SPEED, tween='linear', on_finished=flagMoveGhosts)
                else:
                    player.lives -= 1
                    sounds.pac2.play()
                    if player.lives == 0:
        ...
        if dirs[ghosts[g].dir] == 0 or randint(0,50) == 0:
            ...
            while d == -1:
                ...
                if dirs[rd] == 1:
                    d = rd
            ghosts[g].dir = d
        animate(ghosts[g], pos=(ghosts[g].x + dmoves[ghosts[g].dir][0]*20, ghosts[g].y + dmoves[ghosts[g].dir][1]*20), duration=1/Ghost_SPEED, tween='linear', on_finished=flagMoveGhosts)
...
def ambushPlayer(g, dirs):
    d = ghosts[g].dir
    if player.movex > 0 and dirs[0] == 1: ghosts[g].dir = 1
    if player.movex < 0 and dirs[2] == 1: ghosts[g].dir = 1
